(ns restql.core.request.statement.resolve-chained-values-test
  (:require [clojure.test :refer :all]
            [restql.core.request.statement.resolve-chained-values :refer [resolve-chained-values]]))
  ...
  (testing "Resolve a statement with lists and nested values"
    (is (= {:from :done-resource :with {:name ["clojure" "java"]}}
           (resolve-chained-values {:from :done-resource :with {:name [:resource-id :language :id]}}
                                             {:done [[:resource-id {:body {:language {:id ["clojure" "java"]}}}]]})))

    (is (= {:from :done-resource :with {:name "clojure"}}
           (resolve-chained-values {:from :done-resource :with {:name [:resource-id :language :id]}}
                                             {:done [[:resource-id {:body {:language {:id "clojure"}}}]]})))
    ...
    (is (= {:from :done-resource :with {:name ["clojure" "java"]}}
           (resolve-chained-values {:from :done-resource :with {:name [:resource-id :language :id]}}
                                             {:done [[:resource-id {:body {:language [{:id "clojure"} {:id "java"}]}}]]})))
    ...
    (is (= {:from :done-resource :with {:name ["python" "elixir"]}}
           (resolve-chained-values {:from :done-resource :with {:name [:resource-id :language :xpto :id]}}
                                             {:done [[:resource-id {:body {:language [{:xpto {:id "python"}} {:xpto {:id "elixir"}}]}}]]})))

    (is (= {:from :done-resource :with {:name [["python" "123"] ["elixir" "345"]]}}
           (resolve-chained-values {:from :done-resource :with {:name [:resource-id :language :xpto :id]}}
                                             {:done [[:resource-id {:body {:language [{:xpto {:id ["python" "123"]}} {:xpto {:id ["elixir" "345"]}}]}}]]})))
  ...
  (testing "Resolve with encoder"
    (is (= (binding [*print-meta* true]
           ...
           (binding [*print-meta* true]
             (pr-str (resolve-chained-values {:from :resource-name
